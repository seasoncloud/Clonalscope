#' This function creates new CNV segments list based on 1st round estimation with
...
#'
#' @return New CNV segments generated by merging breakpoints from different clusters.
#' "seg_filtered": A segment table containing chromosome, start and end of each segment.
#'
#' @export
CreateSegtableNoWGS=function(mtx=NULL, barcodes=NULL, features=NULL, Cov_obj=NULL,Zest=NULL,
                             bin_bed=NULL, celltype0=NULL,dir_path=NULL,bin_mtx=NULL,gtf=NULL,
                             plot_seg = TRUE, hmm_states = c(0.5, 1.5, 2), max_qt = 0.95, nmean=500, rm_extreme = 2, adj=-0.5, rds_path=NULL){
  ...
  for(cc in 1:ncol(bin_by_cluster)){ # for each cluster, perform HMM tp obtain segments
    ...
    Obj_filtered=Segmentation_bulk(Obj_filtered=Obj_filtered,
                                   plot_seg=plot_seg, hmm_states = hmm_states, max_qt=max_qt, nmean=nmean, rm_extreme=rm_extreme, adj=adj,rds_path=rds_path)
