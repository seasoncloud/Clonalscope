...
#'
#' @export
MCMCtrim=function(clusteringObj=NULL, burnin=NULL, thinning=1, allele=F){
  ...
  results$Likelihood=results$Likelihood[((0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1)]
    ...
    results$Zall=results$Zall[-c(1:burnin),]
    results$Uall=results$Uall[-c(1:burnin)]
    results$Ucov=results$Ucov[-c(1:burnin)]
    results$Uallele=results$Uallele[-c(1:burnin)]
    results$sigma_cov_all=results$sigma_cov_all[-c(1:burnin)]
    results$sigma_allele_all=results$sigma_allele_all[-c(1:burnin)]
    results$Likelihood=results$Likelihood[-c(1:burnin)]

    # remove thinning
    results$Zall=results$Zall[((0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1),]
    results$Uall=results$Uall[(0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1]
    results$Ucov=results$Ucov[(0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1]
    results$Uallele=results$Uallele[(0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1]
    results$sigma_cov_all=results$sigma_cov_all[(0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1]
    results$sigma_allele_all=results$sigma_allele_all[(0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1]
    results$Likelihood=results$Likelihood[((0:(ceiling(nrow(results$Zall)/thinning)-1))*thinning+1)]
