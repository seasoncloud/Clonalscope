AssignCluster=function(cluster_obj=NULL, mincell=20, cutoff=0.2, allele=FALSE, cna_states_WGS=NULL, rm_extreme=F, cap_corr=1.5, use_weight=F){
  ...
  if(allele==FALSE){
    ...
    Usub2=apply(Usub2,2, function(x) pmax(x, 0.5))

    #Usub2[,which(priors==1.5)]=apply(Usub2[,which(priors==1.5), drop=F], 2, function(x) pmax(x,1))
    #Usub2[,which(priors==0.5)]=apply(Usub2[,which(priors==0.5), drop=F], 2, function(x) pmin(x,1))
...
return(list(Zest=Zest, corrs=corrs[Zest], annot=annot[Zest],Uest=Usub, sigmas_est=sigmas, Zmaj= maj_vote, cutoff=cutoff, U0=cluster_obj$priors$U0,  wU0=cluster_obj$priors$wU0))
  ...
  Usuba=matrix(nrow=nrow(Uall[[length(Uall)]]), ncol=R)
  Usub=matrix(nrow=nrow(Uall[[length(Uall)]]), ncol=R)
  Xir_cov2=apply(Xir_cov, 2, function(x) pmin(x,3))
  for(kk in ind_clusters){
    meanXcov=colMeans(Xir_cov2[which(Zest==kk),, drop=F],  na.rm = T)
    meanXcov[is.na(meanXcov)]=0
    Usubc[kk,]=meanXcov
  ...
  colnames(Usubc)=colnames(Usuba)=colnames(Usub)=colnames(Xir_cov)
  ...
  Usub2=apply(Usubc,2, function(x) pmin(x, cap_corr))
  Usub2=apply(Usub2,2, function(x) pmax(x, 0.5))
