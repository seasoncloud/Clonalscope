...
In this tutorial, we will use a spatial transcriptomics colorecral carcinoma dataset generated by our study (10x Genomics chemistry, will be avaliable upon publication), to demonstrate how to (1) identify tumor/normal cells first, then (2) cluster on tumor cells only, and (3) lastly visualize the clones with spatial spot_datainates.
...
Here, we introduce several ways to identify tumor versue normal cells when paired DNA data is not available.
